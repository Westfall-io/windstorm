...
def galestorm(project_id : str,
                ...
                if oid['@type'].lower() == 'MetadataUsage'.lower():
                    ...
                    if mdid['qualifiedName'] == 'AnalysisTooling::ToolExecution':
                        ...
                        for mdoe in oid['ownedElement']:
                            ...
                            if mdoeid['@type'] == 'ReferenceUsage' and \
                                mdoeid['name'] == 'toolName':
                                if len(mdoeid['ownedElement']) > 1:
                                    ...
                                    if f['value'] == 'Windstorm':
                                        ...
                                        logger.info('            Adding action as valid windstorm analysis: {}'.format(eid['declaredName']))
                                        aj.append(eid)
                else:
    ...
    for a in aj:
        ...
        for var in eid['input']:
            ...
            logger.info('      ---------------------------------')
            logger.info('      Finding actual input values to link to tool variable names.')
            logger.info('      ---------------------------------')
            if toolvarbool:
                if 'ownedElement' in varid:
                    for voe in varid['ownedElement']:
                        ...
                        if voeid['@type'] == 'FeatureChainExpression':
                            ...
                            if 'chainingFeature' in valid:
                                ...
                                for key in chainid['ownedElement']:
                                    ...
                                    if v2['@type'] == 'LiteralInteger':
                                        logger.info('         Value: {}'.format(v2['value']))
                            ...
                            # No chaining feature
                            logger.debug('         Skipping over element: {}'.format(voeid['@type']))
                    ###### END LOOP for each element in owned element
                else:
                    # No chaining feature
    ...
    for (dir_path, dir_names, file_names) in os.walk(in_directory):
        for name in file_names:
            ...
            Path(dir_path.replace(in_directory, out_directory)).mkdir(parents=True, exist_ok=True)
                    ...
                    try:
                        ...
                        if in_directory != out_directory:
                            with open(outfile, 'w') as f2:
                                f2.write(f.read())
                        logger.warning('Skipping file {}/{} because it was not text-based.'.format(dir_path, name))
                        continue
